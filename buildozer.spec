...
# Debug signing – use the stable keystore generated/cached by CI so that
# successive APK builds can be installed as updates (same cert = same app).
# The keystore file is generated by the GitHub Actions workflow on first run
# and cached with a fixed key so it never changes between builds.
